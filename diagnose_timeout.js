...
const testWithTimeout = (path, data = null, testName = "Test") => {
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (error) => {
      ...
      console.log(
        `❌ ${testName} ERROR after: ${errorTime}ms - ${error.message}`
      );
      reject(error);
    });
...
async function runDiagnostics() {
  ...
  try {
    ...
    const simpleReg = await testWithTimeout(
      "/api/auth/register",
      {
    ...
    console.log("\n📋 TEST 3: Complete Registration Data");
    const completeReg = await testWithTimeout(
      "/api/auth/register",
      {
        phoneNumber: "+919876543210",
  ...
  console.log("\n💡 ANALYSIS:");
  console.log(
    "- If health check is fast but registration is slow, issue is in registration logic"
  );
  console.log(
    "- If all endpoints are slow, issue is with server/database connection"
  );
  console.log(
    "- If timeouts occur, server might be hanging on SMS/database operations"
  );
  console.log("\n🔧 NEXT STEPS:");
